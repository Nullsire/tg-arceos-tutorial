...
impl VfsNodeOps for DirNode {
    ...
    fn rename(&self, src_path: &str, dst_path: &str) -> VfsResult {
        ...
        if let Some(src_rest) = src_rest {
            // Source is a deeper path, recurse into child directory
            let child = self.children.read().get(src_name).cloned().ok_or(VfsError::NotFound)?;
            // If dst also has a rest component and the first component matches,
            // strip it too (same-directory rename at a deeper level).
            // Otherwise, it's a cross-directory rename which is not supported.
        ...
        // Leaf level: src_name is the entry to rename in this directory
